...
namespace GrpcAudioStreaming.Server
{
    public class AudioSampleSource : IAudioSampleSource, IDisposable
    {
        ...
        public AudioSampleSource()
        {
        }

        public AudioSampleSource(string nombreCancion)
        {
            ObtenerFileReaderDeCancion(nombreCancion);
            _cancellationTokenSource = new CancellationTokenSource();
        }


        public void ObtenerAudioFormatDeCancion(string cancion)
        {
            _waveFileReader = new WaveFileReader(cancion);
            AudioFormat = _waveFileReader.WaveFormat.ToAudioFormat();
        }

        public void ObtenerFileReaderDeCancion(string cancion)
        {
            _waveFileReader = new WaveFileReader(cancion);
        }
